...
export async function GET(request: NextRequest) {
  try {
    ...
    if (cronSecret && authHeader !== `Bearer ${cronSecret}`) {
      log.error('[cron-scheduled-emails] Unauthorized access attempt')
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }

    log.info('[cron-scheduled-emails] Starting scheduled email processing...')
    ...
    if (error) {
      log.error('[cron-scheduled-emails] Failed to fetch emails:', error)
      return NextResponse.json({ error: 'Database error' }, { status: 500 })
    }

    if (!scheduledEmails || scheduledEmails.length === 0) {
      log.info('[cron-scheduled-emails] No emails to send')
      return NextResponse.json({ success: true, sent: 0 })
    }

    log.info(`[cron-scheduled-emails] Found ${scheduledEmails.length} emails to send`)
      ...
      try {
        ...
        switch (email.email_type) {
          ...
          case 'onboarding_day_7':
            ...
            break
          default:
            log.warn('[cron-scheduled-emails] Unknown email type:', email.email_type)
            continue
        }
        ...
        if (result.success) {
          ...
          results.sent++
          log.info(`[cron-scheduled-emails] Sent ${email.email_type} to user ${email.user_id}`)
        } else {
          results.failed++
          results.errors.push(`${email.email_type} for user ${email.user_id}: ${result.error}`)
          log.error(`[cron-scheduled-emails] Failed to send ${email.email_type}:`, result.error)
        }
      } catch (error) {
        results.failed++
        results.errors.push(`${email.email_type}: ${error}`)
        log.error('[cron-scheduled-emails] Unexpected error:', error)
      }
    ...
    log.info('[cron-scheduled-emails] Processing complete:', results)
    ...
    log.error('[cron-scheduled-emails] Fatal error:', error)
    return NextResponse.json(
      {
