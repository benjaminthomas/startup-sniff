...
function getPlanConfig(planType: string | undefined) {
  const plan = (planType || 'explorer').toLowerCase();
  ...
  switch (plan) {
    case 'explorer':
      return {
        ...
        icon: Target,
        badgeClasses: 'border-slate-300 bg-slate-50 text-slate-700 hover:bg-slate-100',
    ...
    case 'founder':
      return {
        ...
        icon: Zap,
        badgeClasses: 'bg-blue-500 text-white border-blue-500 hover:bg-blue-600',
    ...
    case 'growth':
      return {
        ...
        variant: 'default' as const,
        icon: Crown,
        badgeClasses: 'bg-gradient-to-r from-purple-600 to-pink-600 text-white border-purple-600 hover:from-purple-700 hover:to-pink-700',
    ...
    <div className="flex items-center justify-between w-full">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2">
          <Badge variant={planConfig.variant} className={`flex items-center gap-1.5 px-2.5 py-1 ${planConfig.badgeClasses}`}>
            <planConfig.icon className={`h-3.5 w-3.5 ${planConfig.iconColor}`} />
            ...
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="relative h-8 w-8 rounded-full">
                <Avatar className="h-8 w-8">
                  <AvatarFallback className="text-gray-900 hover:text-gray-900">{getInitials(user?.full_name)}</AvatarFallback>
                </Avatar>
              </Button>
            </DropdownMenuTrigger>
